...
const ProcessingOrder = () => {
  ...
  const {setProcessingOrderCount} = useGlobalData();
  ...
  const [fetchProcessingList, {loading: processingLoading}] = useLazyQuery(
    SELLER_ORDER_LIST,
    {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={allProcessingOrder}
        keyExtractor={item => item?.increment_id}
        renderItem={({item, index}) => (
          <AllOrdersListing
            navigation={navigation}
            productData={allProcessingOrder}
            index={index}
            item={item}
            isAllOrders={false}
        ...
        ListEmptyComponent={!processingLoading && <NoRecordFound />}
        contentContainerStyle={[
          styles.list,
          {flex: allProcessingOrder?.length > 0 ? null : 1},
        ]}
